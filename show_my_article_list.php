  ...
  <div class="container">
    <div class="row"> 
      ...
      <div id="content" class="8u skel-cell-important" style='width: 100%'>
		...
		$res->execute(array($_SESSION['login_user']));
		...
		echo "<td>".$row[$i]['srcn']."</td>";
		echo "<td>".$row[$i]['srch']."</td>";
		echo "<td>".$row[$i]['content']."</td>";
		echo '<td><a href="update_article_index.php?id='.$row[$i]['id'].'&datetm='.$row[$i]['datetm'].'">編輯</a>
				<a href="delete_article.php?id='.$row[$i]['id'].'&datetm='.$row[$i]['datetm'].'">刪除</a></td>';
		echo "</tr>";
