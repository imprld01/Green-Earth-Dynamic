	  ...
	  <div class="container">
		<div class="row"> 
		  ...
		  <div id="content" class="8u skel-cell-important" style='width: 100%'>
			...
			<form action="tree_data_insert.php" method="post">
			樹名: <input type="text" name="name" required/>
			學名: <input type="text" name="scin" required />
			其他名字: <input type="text" name="comn" required />
			描述: <input type="text" name="description" required />
			圖片網址: <input type="text" name="picsrc" required />
			<br></br>
			<input type="submit" value="新增樹木資料!" required />
			</form>
			<br></br>
			...
			;
			$row=$res->fetchAll();
			echo "<table border=1 style='border-collapse:collapse; border-color:#82b440;  border:#FFAE7F 2px solid;'>
			<tr style='	border-bottom: solid 1px #82b440;'>
			<th class='default'>樹名</th>
			<th>學名</th>
			<th>其他名稱</th>
			<th>描述</th>
			<th>圖片網址</th>
			...
			echo "<td>".$row[$i]['scin']."</td>";
			echo "<td>".$row[$i]['comn']."</td>";	
			echo "<td>".$row[$i]['description']."</td>";	
			echo "<td><div><img src=".$row[$i]['picsrc']." width=50px height=25px></div></td>";
			echo "</tr>";
			}
			echo "</table>";
	   ?>
